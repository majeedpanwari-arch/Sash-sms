"""
Rings & Access Codes Management Routes
"""
...
def toggle_ring(ring_id):
    """Toggle active status of a Ring."""
    ring = Ring.query.get_or_404(ring_id)
...
def create_codes():
    ...
    if request.method == 'POST':
        ...
        for _ in range(count):
            ...
            # Ensure unique code and number combination
...
@admin_required
def export_codes():
    """Export access codes to CSV file."""
...
def verify_code():
    """Code Verification Page for Clients."""
    ...
    if code and number:
        ...
        if access_code:
            ...
            if not ring or not ring.is_active:
                return render_template('verify.html',
                                       error='Ring is currently unavailable.')
...
def api_activate():
    """API to activate code after purchase."""
    data = request.json
    code = data.get('code', '').strip()
    number = data.get('number', '').strip()
