...
def agent_add_numbers():
    ...
    if request.method == 'POST':
        ...
        # Calculate numbers held by agent and verify limit
...
def agent_create_client():
    ...
    if request.method == 'POST':
        ...
        client = User(
            ...
            company=company,
            country=country,
            agent_id=current_user.id,  # Link client to agent
            is_active=True
        )
...
def download_added_numbers(numbers_list, agent_username):
    """Download text file containing added numbers"""
    content = "\n".join(numbers_list)
...
def download_client_numbers(numbers_list, client_username):
    """Download text file containing client numbers"""
    content = "\n".join(numbers_list)
...
def telegram_test():
    ...
    message = request.form.get('message', 'Test message from Sash SMS').strip()
...
def telegram_send_otp():
    ...
    try:
        ...
        response = requests.post(url, json={
            ...
            'text': f'Your Sash SMS OTP Code: {code}'
        }, timeout=10)
