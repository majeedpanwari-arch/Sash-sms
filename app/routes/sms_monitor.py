"""
sms_monitor.py  —  Sash SMS Monitor (FIXED V2)
======================================
Fetches messages from external sources (5 sources)
and automatically forwards them to reserved numbers.

FIX LIST:
  1. FIX: Date vs Number Confusion — Updated _detect_abyss_indexes to distinguish date first and exclude it from phone number lookup.
  2. FIX: Number format in ABYSS — Ensured fetching complete number from the correct index.
"""
...
# ════════════════════════════════════════════════════════════
# SOURCE ABYSS (Sash SMS Panel — session/captcha + auto index detection)
# ════════════════════════════════════════════════════════════
...
def _detect_abyss_indexes(rows):
    """Smartly infer indexes for date, number, and message to avoid overlap."""
    ...
    # 1. Search for date first (YYYY-MM-DD format)
    ...
    # 2. Search for phone number (digits only, not the date, proper length)
    max_digit_len = 0
    for i, cell in enumerate(sample):
        if i == idx_date: continue # Completely exclude date column
        
        cell_str = _clean_html(cell)
        digits = _clean_num(cell_str)
        
        # Exclude digits resembling dates without separators (e.g. 20260418001936 or 20260418)
        if digits.startswith("202") and len(digits) in (8, 14):
            continue
        
        # Ensure number has valid length for phone number (typically 8-15 digits)
        if 8 <= len(digits) <= 15:
    ...
    # 3. Search for message body (longest text, not date or number)
    max_text_len = 0
    for i, cell in enumerate(sample):
...
def _abyss_fetch_internal():
    ...
    for row in rows:
        ...
        # Ensure "number" is not just a numeric date string
...
def index():
    ...
    sources_info = [
        {"key": "abyss",   "label": "Sash SMS", "type": "Session"},
